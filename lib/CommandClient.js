...
class CommandClient extends Client {
   ...
   */
  constructor({ prefix = '?', mentionable = true, ignoreBots = true,
                additionalConverters = {} } = {}, clientOptions = {}) {
    super(clientOptions);
  ...
  parseArgs(args, message) {
    ...
    if (items.length < minimumArguments) {
      ...
      throw new errors.RequiredArgumentMissing(`${firstMissingArgument.name} is a required argument that is missing.`);
    } else if (items.length > args.length) {
      throw new errors.TooManyArguments('There are too many arguments for me to parse.');
    }
